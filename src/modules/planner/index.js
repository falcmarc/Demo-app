...
export default function Planner() {
  ...
  el.innerHTML = `
    ...
    <div class="small" style="margin-bottom:8px">Seleziona un piatto per ogni giorno (1 pasto/giorno per semplicità)</div>
  ...
  const render = () => {
    ...
    days.forEach((d,idx)=>{
      ...
      sel.innerHTML = `<option value="">— scegli ricetta —</option>` + RECIPES.map(r=>`<option value="${r.id}">${r.name}</option>`).join('');
      sel.value = plan[idx].meal || '';
      const serv = document.createElement('input'); serv.type='number'; serv.min='1'; serv.className='input'; serv.value = plan[idx].servings || 2;

      sel.addEventListener('change', ()=>{
        const p = get(); p[idx].meal = sel.value || null; set(p);
      });
      serv.addEventListener('change', ()=>{
        const p = get(); p[idx].servings = Math.max(1, +serv.value||1); set(p);
      });
