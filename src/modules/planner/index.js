...
export default function Planner() {
  ...
  function filterRecipes() {
    ...
    const activeTags = tagsBox
      ? Array.from(tagsBox.querySelectorAll('button[data-active="1"]')).map(b => b.textContent)
      : [];
    return RECIPES
      .filter(allowDiet)
  ...
  function render() {
    ...
    days.forEach((d, idx) => {
      ...
      serv.type = 'number';
      serv.min = '1';
      serv.className = 'input';
      serv.value = plan[idx].servings ? plan[idx].servings : eq;

      sel.addEventListener('change', () => {
        const p = get();
        p[idx].meal = sel.value || null;
        set(p);
      });

      serv.addEventListener('change', () => {
        const p = get();
        p[idx].servings = Math.max(1, +serv.value || eq);
        set(p);
      });
