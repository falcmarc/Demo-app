...
export default function List(){
  ...
  const eq       = Math.max(1, equivalents(settings));
  ...
  el.innerHTML = `
    ...
    <div class="small">Persone eq: <strong>${Math.round(eq*10)/10}</strong> · Budget: € <strong>${settings.budget}</strong> · Dieta: <strong>${settings.diet}</strong></div>
  ...
  ul.innerHTML = items.map(x=>`<li><strong>${x.item}</strong> — ${Math.round(x.qty)} ${x.unit}</li>`).join('')
    || `<li class="small">Completa il planner per generare la lista.</li>`;
