...
export default function Start() {
  ...
  el.innerHTML = `
    ...
    <div style="display:grid; gap:12px; grid-template-columns: repeat(2, minmax(220px, 1fr)); max-width:920px">
      ...
      <div>
        ...
        <input id="budget" class="input" type="number" min="0" value="${S.budget||0}" />
      </div>

      <div>
        <label class="small">Adulti</label>
        <input id="adults" class="input" type="number" min="0" value="${S.adults||0}" />
      </div>

      <div>
        <label class="small">Bambini</label>
        <input id="kids" class="input" type="number" min="0" value="${S.kids||0}" />
      </div>

      <div style="grid-column: 1 / -1">
        <label class="small">Età bambini (auto‑aggiornate)</label>
        <div id="kidsAges" style="display:flex; gap:8px; flex-wrap:wrap"></div>
      </div>

      <div>
        <label class="small">Adulti che non pranzano</label>
        <input id="adultsSkip" class="input" type="number" min="0" value="${S.adultsSkipLunch||0}" />
      </div>

      <div>
        <label class="small">Bambini che non pranzano</label>
        <input id="kidsSkip" class="input" type="number" min="0" value="${S.kidsSkipLunch||0}" />
      </div>
  ...
  el.querySelector('#save').addEventListener('click', () => {
    ...
    save(out);
    el.querySelector('#msg').textContent = 'Salvato ✔';
    // Vai al planner subito dopo il salvataggio
